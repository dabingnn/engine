cc3d.extend(cc3d, function () {
    ...
    var lmCamera;
    var tempVec = new cc.Vec3();
    var bounds = new cc3d.BoundingBox();
    ...
    Lightmapper.prototype = {
        ...
        calculateLightmapSize: function (node) {
            ...
            return Math.min(cc3d.math.nextPowerOfTwo(totalArea * sizeMult), this.scene.lightmapMaxResolution || maxSize);
        },

        /**
         * @function
         * @name cc3d.Lightmapper#bake
         * @description Generates and applies the lightmaps.
         * @param {cc3d.Entity} nodes An array of models to render lightmaps for. If not supplied, full scene will be baked.
        ...
        bake: function (nodes, mode) {
            ...
            var passCount = 1;
            if (mode === undefined) mode = cc3d.BAKE_COLORDIR;
            if (mode === cc3d.BAKE_COLORDIR) passCount = 2;
            var pass;
            ...
            var tex;
            var instances;
            var blackTex = new cc3d.Texture(this._device, {
                width: 4,
                height: 4,
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                for (pass = 0; pass < passCount; pass++) {
                    ...
                    tex._minFilter = cc3d.FILTER_NEAREST;
                    tex._magFilter = cc3d.FILTER_NEAREST
                    lmaps[pass].push(tex);
                ...
                if (!texPool[size]) {
                    ...
                    tex2._minFilter = cc3d.FILTER_NEAREST;
                    tex2._magFilter = cc3d.FILTER_NEAREST;
                ...
                if (sceneLights[i]._enabled) {
                    ...
                    if ((mask & maskLightmap) !== 0) {
                        ...
                        sceneLights[i].shadowUpdateMode =
                            sceneLights[i].getType() === cc3d.LIGHTTYPE_DIRECTIONAL ? cc3d.SHADOWUPDATE_REALTIME : cc3d.SHADOWUPDATE_THISFRAME;
            ...
            for (node = 0; node < allNodes.length; node++) {
                ...
                for (i = 0; i < rcv.length; i++) {
                    rcv[i]._shaderDefs &= ~(cc3d.SHADERDEF_LM | cc3d.SHADERDEF_DIRLM);
                    //rcv[i].mask |= cc3d.MASK_DYNAMIC;
                    //rcv[i].mask &= ~cc3d.MASK_LIGHTMAP;
                }
            }
            ...
            for (i = 0; i < origCasters.length; i++) {
                m = origCasters[i];
                instanceClone = new cc3d.MeshInstance(m.node, m.mesh, m.material);
                for (prop in m) {
            ...
            for (pass = 0; pass < passCount; pass++) {
                if (!passMaterial[pass]) {
                    lmMaterial = new cc3d.StandardMaterial();
                    ...
                    lmMaterial.cull = cc3d.CULLFACE_NONE;
            ...
            for (node = 0; node < nodes.length; node++) {
                ...
                var nbounds = new cc3d.BoundingBox();
                nbounds.copy(bounds);
                ...
                for (i = 0; i < rcv.length; i++) {
                    // patch meshInstance
                    m = rcv[i];
                    m._shaderDefs &= ~(cc3d.SHADERDEF_LM | cc3d.SHADERDEF_DIRLM); // disable LM define, if set, to get bare ambient on first pass
                ...
                for (pass = 0; pass < passCount; pass++) {
                    lm = lmaps[pass][node];
                    targ = new cc3d.RenderTarget(device, lm, {
                        depth: false
                    });
            ...
            for (i = 0; i < lights.length; i++) {
                ...
                if (lights[i].getType() !== cc3d.LIGHTTYPE_DIRECTIONAL) {
                ...
                if (lights[i].getType() === cc3d.LIGHTTYPE_SPOT) {
                    light = lights[i];
                ...
                for (node = 0; node < nodes.length; node++) {
                    ...
                    if (lights[i].getType() === cc3d.LIGHTTYPE_DIRECTIONAL) {
                        ...
                        lmCamera.setProjection(cc3d.PROJECTION_ORTHOGRAPHIC);
                    ...
                    for (pass = 0; pass < passCount; pass++) {
                        ...
                        for (j = 0; j < rcv.length; j++) {
                            ...
                            m._shaderDefs |= cc3d.SHADERDEF_LM; // force using LM even if material doesn't have it
                        }
                    }
            ...
            for (node = 0; node < nodes.length; node++) {
                ...
                for (pass = 0; pass < passCount; pass++) {
                    ...
                    for (i = 0; i < numDilates2x; i++) {
                        ...
                        cc3d.drawQuadWithShader(device, targTmp, dilateShader);

                        constantTexSource.setValue(texTmp);
                        cc3d.drawQuadWithShader(device, targ, dilateShader);
                    }
                    ...
                    for (i = 0; i < rcv.length; i++) {
                        ...
                        if (pass === PASS_DIR) rcv[i]._shaderDefs |= cc3d.SHADERDEF_DIRLM;
                    }
                    sceneLmaps[pass] = lm;
            ...
            stats.totalRenderTime = cc3d.now() - startTime;
