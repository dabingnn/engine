cc3d.extend(cc3d, function () {
    ...
    var scaleShift = new cc.Mat4().mul2(
        new cc.Mat4().setTranslate(0.5, 0.5, 0.5),
        new cc.Mat4().setScale(0.5, 0.5, 0.5)
    );
    ...
    var pixelOffset = new cc.Vec2v2();
    var blurScissorRect = {x: 1, y: 1, z: 0, w: 0};

    var shadowCamView = new cc.Mat4();
    var shadowCamViewProj = new cc.Mat4();
    var c2sc = new cc.Mat4();

    var viewInvMat = new cc.Mat4();
    var viewMat = new cc.Mat4();
    var viewMat3 = new cc.Mat3();
    var viewProjMat = new cc.Mat4();
    var frustumDiagonal = new cc.Vec3();
    var tempSphere = {center: null, radius: 0};
    var meshPos;
    var visibleSceneAabb = new cc3d.BoundingBox();
    ...
    var frustumPoints = [];
    for (var i = 0; i < 8; i++) {
        frustumPoints.push(new cc.Vec3());
    }
    ...
    function _getFrustumPoints(camera, farClip, points) {
        ...
        var x, y;
        if (projection === cc3d.PROJECTION_PERSPECTIVE) {
    ...
    var intersectCache = {
        temp: [new cc.Vec3(), new cc.Vec3(), new cc.Vec3()],
    ...
    function getShadowFormat(shadowType) {
        if (shadowType === cc3d.SHADOW_VSM32) {
            return cc3d.PIXELFORMAT_RGBA32F;
        } else if (shadowType === cc3d.SHADOW_VSM16) {
            return cc3d.PIXELFORMAT_RGBA16F;
        }
        return cc3d.PIXELFORMAT_R8_G8_B8_A8;
    }

    function getShadowFiltering(device, shadowType) {
        if (shadowType === cc3d.SHADOW_DEPTH) {
            return cc3d.FILTER_NEAREST;
        } else if (shadowType === cc3d.SHADOW_VSM32) {
            return device.extTextureFloatLinear ? cc3d.FILTER_LINEAR : cc3d.FILTER_NEAREST;
        } else if (shadowType === cc3d.SHADOW_VSM16) {
            return device.extTextureHalfFloatLinear ? cc3d.FILTER_LINEAR : cc3d.FILTER_NEAREST;
        }
    ...
    function createShadowMap(device, width, height, shadowType) {
        ...
        var shadowMap = new cc3d.Texture(device, {
            format: format,
            width: width,
            height: height,
            autoMipmap: false
        ...
        shadowMap.addressU = cc3d.ADDRESS_CLAMP_TO_EDGE;
        shadowMap.addressV = cc3d.ADDRESS_CLAMP_TO_EDGE;
        return new cc3d.RenderTarget(device, shadowMap, true);
    }

    function createShadowCubeMap(device, size) {
        var cubemap = new cc3d.Texture(device, {
        ...
        cubemap.minFilter = cc3d.FILTER_NEAREST;
        cubemap.magFilter = cc3d.FILTER_NEAREST;
        cubemap.addressU = cc3d.ADDRESS_CLAMP_TO_EDGE;
        cubemap.addressV = cc3d.ADDRESS_CLAMP_TO_EDGE;
        var targets = [];
        for (var i = 0; i < 6; i++) {
            var target = new cc3d.RenderTarget(device, cubemap, {
                face: i,
                depth: true
            });
            targets.push(target);
    ...
    function createShadowCamera(device, shadowType) {
        ...
        var flags = cc3d.CLEARFLAG_DEPTH;
        if (!device.extDepthTexture) flags |= cc3d.CLEARFLAG_COLOR;

        var shadowCam = new cc3d.Camera();
        shadowCam.setClearOptions({
            color: (shadowType > cc3d.SHADOW_DEPTH ? [0, 0, 0, 0] : [1.0, 1.0, 1.0, 1.0]),
            depth: 1.0,
            flags: flags
        });
    ...
    function getShadowMapFromCache(device, res, mode, layer) {
        ...
        if (!shadowBuffer) {
            shadowBuffer = createShadowMap(device, res, res, mode ? mode : cc3d.SHADOW_DEPTH);
    ...
    function createShadowBuffer(device, light) {
        var shadowBuffer;
        if (light.getType() === cc3d.LIGHTTYPE_POINT) {
            if (light._shadowType > cc3d.SHADOW_DEPTH) light._shadowType = cc3d.SHADOW_DEPTH; // no VSM point lights yet
    ...
    /**
     * @private
     * @name cc3d.ForwardRenderer
     * @class The forward renderer render scene objects.
     * @description Creates a new forward renderer object.
     * @param {cc3d.GraphicsDevice} graphicsDevice The graphics device used by the renderer.
     */
    ...
    cc3d.extend(ForwardRenderer.prototype, {
        ...
        sortCompare: function (drawCallA, drawCallB) {
            ...
            return drawCallB._key[cc3d.SORTKEY_FORWARD] - drawCallA._key[cc3d.SORTKEY_FORWARD]; // based on key
        },

        depthSortCompare: function (drawCallA, drawCallB) {
            return drawCallB._key[cc3d.SORTKEY_DEPTH] - drawCallA._key[cc3d.SORTKEY_DEPTH];
        },
        ...
        dispatchSpotLight: function (scene, scope, spot, cnt) {
            ...
            if (spot.getCastShadows()) {
                var bias;
                if (spot._shadowType > cc3d.SHADOW_DEPTH) {
                    bias = -0.00001 * 20;
                } else {
                    bias = spot._shadowBias * 20; // approx remap from old bias values
                    if (this.device.extStandardDerivatives) bias *= -100;
                }
                var normalBias = spot._shadowType > cc3d.SHADOW_DEPTH ?
        ...
        dispatchLocalLights: function (scene, mask, usedDirLights, staticLightList) {
            ...
            var pnts = localLights[cc3d.LIGHTTYPE_POINT - 1];
            var spts = localLights[cc3d.LIGHTTYPE_SPOT - 1];
            ...
            if (staticLightList) {
                ...
                while (point && point.getType() === cc3d.LIGHTTYPE_POINT) {
                    if (!(point.mask & mask)) {
                        staticId++;
                        point = staticLightList[staticId];
                        continue;
            ...
            if (staticLightList) {
                ...
                while (spot) { // && spot.getType()===cc3d.LIGHTTYPE_SPOT) {
                    if (!(spot.mask & mask)) {
                        staticId++;
                        spot = staticLightList[staticId];
                        continue;
        ...
        cull: function (camera, drawCalls) {
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (!drawCall.command) {
                    ...
                    if (drawCall.layer > cc3d.LAYER_FX) {
                        if (drawCall.cull) {
            ...
            // #ifdef PROFILER
            this._cullTime += cc3d.now() - cullTime;
            // #endif
        ...
        calculateSortDistances: function (drawCalls, camPos, camFwd, frontToBack) {
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (drawCall.layer <= cc3d.scene.LAYER_FX) continue; // Only alpha sort mesh instances in the main world
                btype = drawCall.material.blendType;
                if (btype !== cc3d.BLEND_NONE) {
                ...
                if (frontToBack && btype === cc3d.BLEND_NONE) {
                    meshPos = drawCall.aabb.center.data;
                    tempx = meshPos[0] - camPos[0];
                    tempy = meshPos[1] - camPos[1];
                    tempz = meshPos[2] - camPos[2];
            ...
            // #ifdef PROFILER
            this._sortTime += cc3d.now() - sortTime;
            // #endif
        },
        ...
        updateCpuSkinMatrices: function (drawCalls) {
            ...
            // #ifdef PROFILER
            this._skinTime += cc3d.now() - skinTime;
            // #endif
        },
        ...
        prepareInstancing: function (device, drawCalls, keyType, shaderType) {
            ...
            if (device.enableAutoInstancing) {
                for (i = 0; i < drawCallsCount - 1; i++) {
                    ...
                    if (drawCalls[next].mesh === mesh && drawCalls[next]._key[keyType] === key) {
                        ...
                        data._buffer = cc3d._autoInstanceBuffer;
                        i = next - 1;
                    }
                    offset += autoInstances * 16;
                }
                if (offset > 0) cc3d._autoInstanceBuffer.unlock();
            }
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (meshInstance.instancingData) {
                    if (!(meshInstance._shaderDefs & cc3d.SHADERDEF_INSTANCING)) { // TODO: FIX, THIS IS DANGEROUS
                        meshInstance._shaderDefs |= cc3d.SHADERDEF_INSTANCING;
                        meshInstance._shader[shaderType] = null;
                    }
                    if (!meshInstance.instancingData._buffer) {
                        meshInstance.instancingData._buffer = new cc3d.VertexBuffer(device, cc3d._instanceVertexFormat,
                            meshInstance.instancingData.count, meshInstance.instancingData.usage, meshInstance.instancingData.buffer);
                    }
                } else {
                    if (meshInstance._shaderDefs & cc3d.SHADERDEF_INSTANCING) {
                        meshInstance._shaderDefs &= ~cc3d.SHADERDEF_INSTANCING;
                        meshInstance._shader[shaderType] = null;
                    }
                }
            ...
            // #ifdef PROFILER
            this._instancingTime += cc3d.now() - instancingTime;
            // #endif
        },
        ...
        drawInstance: function (device, meshInstance, mesh, style, normal) {
            ...
            if (instancingData) {
                ...
                if (instancingData._buffer === cc3d._autoInstanceBuffer) {
        ...
        findShadowShader: function (meshInstance, type, shadowType) {
            ...
            return this.library.getProgram('depthrgba', {
                ...
                point: type !== cc3d.LIGHTTYPE_DIRECTIONAL,
        ...
        renderShadows: function (device, camera, drawCalls, lights) {
            // #ifdef PROFILER
            var shadowMapStartTime = cc3d.now();
            // #endif
            ...
            for (i = 0; i < lights.length; i++) {
                ...
                if (light.getCastShadows() && light.getEnabled() && light.shadowUpdateMode !== cc3d.SHADOWUPDATE_NONE) {
                    ...
                    if (type === cc3d.LIGHTTYPE_DIRECTIONAL) {
                        ...
                        shadowCam.setProjection(cc3d.PROJECTION_ORTHOGRAPHIC);
                    ...
                    } else if (type === cc3d.LIGHTTYPE_SPOT) {
                    ...
                    if (light.shadowUpdateMode === cc3d.SHADOWUPDATE_THISFRAME) light.shadowUpdateMode = cc3d.SHADOWUPDATE_NONE;
                    ...
                    for (pass = 0; pass < passes; pass++) {
                        ...
                        this.setCamera(shadowCam, type !== cc3d.LIGHTTYPE_POINT);
                        ...
                        smode = shadowType + (type !== cc3d.LIGHTTYPE_DIRECTIONAL ? numShadowModes : 0);
                        this.sortDrawCalls(culled, this.depthSortCompare, cc3d.SORTKEY_DEPTH, true);
                        this.prepareInstancing(device, culled, cc3d.SORTKEY_DEPTH, cc3d.SHADER_SHADOW + smode);
                        ...
                        for (j = 0, numInstances = culled.length; j < numInstances; j++) {
                            ...
                            // set shader
                            shadowShader = meshInstance._shader[cc3d.SHADER_SHADOW + smode];
                            if (!shadowShader) {
                                shadowShader = this.findShadowShader(meshInstance, type, shadowType);
                                meshInstance._shader[cc3d.SHADER_SHADOW + smode] = shadowShader;
                                meshInstance._key[cc3d.SORTKEY_DEPTH] = getDepthKey(meshInstance);
                            }
                    ...
                    if (light._shadowType > cc3d.SHADOW_DEPTH) {
                        ...
                        if (filterSize > 1) {
                            ...
                            var blurShader = (light._shadowType === cc3d.SHADOW_VSM8 ? this.blurPackedVsmShader : this.blurVsmShader)[blurMode][filterSize];
                            if (!blurShader) {
                                this.blurVsmWeights[filterSize] = gaussWeights(filterSize);
                                var chunks = cc3d.shaderChunks;
                                (light._shadowType === cc3d.SHADOW_VSM8 ? this.blurPackedVsmShader : this.blurVsmShader)[blurMode][filterSize] = blurShader =
                                    chunks.createShaderFromCode(this.device, chunks.fullscreenQuadVS,
                                        "#define SAMPLES " + filterSize + "\n" +
                                        (light._shadowType === cc3d.SHADOW_VSM8 ? this.blurPackedVsmShaderCode : this.blurVsmShaderCode)
                                            [blurMode], "blurVsm" + blurMode + "" + filterSize + "" + (light._shadowType === cc3d.SHADOW_VSM8));
                            }
                            ...
                            if (blurMode === cc3d.BLUR_GAUSSIAN) this.weightId.setValue(this.blurVsmWeights[filterSize]);
                            cc3d.drawQuadWithShader(device, tempRt, blurShader, null, blurScissorRect);
                            ...
                            pixelOffset.x = 0.0;
                            this.pixelOffsetId.setValue(pixelOffset.data);
                            cc3d.drawQuadWithShader(device, origShadowMap, blurShader, null, blurScissorRect);
                        }
                    }
                }
            }
            // #ifdef PROFILER
            this._shadowMapTime = cc3d.now() - shadowMapStartTime;
            // #endif
        },
        ...
        filterDepthMapDrawCalls: function (drawCalls) {
            ...
            for (var i = 0; i < drawCalls.length; i++) {
                meshInstance = drawCalls[i];
                if (!meshInstance.command && meshInstance.drawToDepth && meshInstance.material.blendType === cc3d.BLEND_NONE) {
        ...
        renderDepth: function (device, camera, drawCalls) {
            ...
            if (camera._renderDepthRequests) {
                ...
                var drawCallsCount = drawCalls.length;
                this.sortDrawCalls(drawCalls, this.depthSortCompare, cc3d.SORTKEY_DEPTH, true);
                this.prepareInstancing(device, drawCalls, cc3d.SORTKEY_DEPTH, cc3d.SHADER_DEPTH);
                ...
                if (!camera._depthTarget) {
                    var colorBuffer = new cc3d.Texture(device, {
                        format: cc3d.PIXELFORMAT_R8_G8_B8_A8,
                        width: width,
                        height: height
                    });
                    colorBuffer.minFilter = cc3d.FILTER_NEAREST;
                    colorBuffer.magFilter = cc3d.FILTER_NEAREST;
                    colorBuffer.addressU = cc3d.ADDRESS_CLAMP_TO_EDGE;
                    colorBuffer.addressV = cc3d.ADDRESS_CLAMP_TO_EDGE;
                    camera._depthTarget = new cc3d.RenderTarget(device, colorBuffer, {
                        depth: true,
                ...
                for (i = 0; i < drawCallsCount; i++) {
                    ...
                    this.setSkinning(device, meshInstance, material)
                    // set shader
                    depthShader = meshInstance._shader[cc3d.SHADER_DEPTH];
                    if (!depthShader) {
                        depthShader = this.findDepthShader(meshInstance);
                        meshInstance._shader[cc3d.SHADER_DEPTH] = depthShader;
                        meshInstance._key[cc3d.SORTKEY_DEPTH] = getDepthKey(meshInstance);
                    }
            ...
            // #ifdef PROFILER
            this._depthMapTime = cc3d.now() - startTime;
            // #endif
        },
        ...
        renderForward: function (device, camera, drawCalls, scene) {
            ...
            // #ifdef PROFILER
            var forwardStartTime = cc3d.now();
            // #endif

            this.sortDrawCalls(drawCalls, this.sortCompare, cc3d.SORTKEY_FORWARD, !this.frontToBack);
            this.prepareInstancing(device, drawCalls, cc3d.SORTKEY_FORWARD, cc3d.SHADER_FORWARD);
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (drawCall.command) {
                    ...
                    if (material !== prevMaterial) {
                        ...
                        if (!drawCall._shader[cc3d.SHADER_FORWARD] || drawCall._shaderDefs !== objDefs) {
                            if (!drawCall.isStatic) {
                                drawCall._shader[cc3d.SHADER_FORWARD] = material.variants[objDefs];
                                if (!drawCall._shader[cc3d.SHADER_FORWARD]) {
                                    material.updateShader(device, scene, objDefs);
                                    drawCall._shader[cc3d.SHADER_FORWARD] = material.variants[objDefs] = material.shader;
                                }
                            } else {
                                material.updateShader(device, scene, objDefs, drawCall._staticLightList);
                                drawCall._shader[cc3d.SHADER_FORWARD] = material.shader;
                            }
                            drawCall._shaderDefs = objDefs;
                        }
                        device.setShader(drawCall._shader[cc3d.SHADER_FORWARD]);
                        ...
                        if (stencilFront || stencilBack) {
                            ...
                            if (stencilFront === stencilBack) {
                                ...
                                if (stencilFront) {
                                    ...
                                    // default front
                                    device.setStencilFuncFront(cc3d.FUNC_ALWAYS, 0, 0xFF);
                                    device.setStencilOperationFront(cc3d.STENCILOP_KEEP, cc3d.STENCILOP_KEEP, cc3d.STENCILOP_KEEP);
                                }
                                if (stencilBack) {
                                    // set back
                                    ...
                                    // default back
                                    device.setStencilFuncBack(cc3d.FUNC_ALWAYS, 0, 0xFF);
                                    device.setStencilOperationBack(cc3d.STENCILOP_KEEP, cc3d.STENCILOP_KEEP, cc3d.STENCILOP_KEEP);
                                }
                            }
                        } else {
            ...
            // #ifdef PROFILER
            this._forwardTime = cc3d.now() - forwardStartTime;
            // #endif
        },
        ...
        sortLights: function (scene) {
            ...
            for (i = 0; i < lights.length; i++) {
                light = lights[i];
                if (light.getEnabled()) {
                    if (light.getType() === cc3d.LIGHTTYPE_DIRECTIONAL) {
                        scene._globalLights.push(light);
                    } else {
                        scene._localLights[light.getType() === cc3d.LIGHTTYPE_POINT ? 0 : 1].push(light);
                    }
                }
            }
            return lights;
        ...
        setupInstancing: function (device) {
            if (!cc3d._instanceVertexFormat) {
                var formatDesc = [
                    {semantic: cc3d.SEMANTIC_TEXCOORD2, components: 4, type: cc3d.ELEMENTTYPE_FLOAT32},
                    {semantic: cc3d.SEMANTIC_TEXCOORD3, components: 4, type: cc3d.ELEMENTTYPE_FLOAT32},
                    {semantic: cc3d.SEMANTIC_TEXCOORD4, components: 4, type: cc3d.ELEMENTTYPE_FLOAT32},
                    {semantic: cc3d.SEMANTIC_TEXCOORD5, components: 4, type: cc3d.ELEMENTTYPE_FLOAT32},
                ];
                cc3d._instanceVertexFormat = new cc3d.VertexFormat(device, formatDesc);
            }
            if (device.enableAutoInstancing) {
                if (!cc3d._autoInstanceBuffer) {
                    cc3d._autoInstanceBuffer = new cc3d.VertexBuffer(device, cc3d._instanceVertexFormat, device.autoInstancingMaxObjects, cc3d.BUFFER_DYNAMIC);
                    cc3d._autoInstanceBufferData = new Float32Array(cc3d._autoInstanceBuffer.lock());
                }
            }
        },
        ...
        prepareStaticMeshes: function (device, scene) {
            ...
            var minv, maxv;
            var minVec = new cc.Vec3();
            var maxVec = new cc.Vec3();
            var triAabb = new cc3d.BoundingBox();
            var localLightBounds = new cc3d.BoundingBox();
            var invMatrix = new cc.Mat4();
            var triLightComb = [];
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (!drawCall.isStatic) {
                    ...
                    // #ifdef PROFILER
                    subCullTime = cc3d.now();
                    // #endif
                    aabb = drawCall.aabb;
                    staticLights.length = 0;
                    for (lightTypePass = cc3d.LIGHTTYPE_POINT; lightTypePass <= cc3d.LIGHTTYPE_SPOT; lightTypePass++) {
                        for (j = 0; j < lights.length; j++) {
                            ...
                            if (light.getEnabled()) {
                                if (light.mask & drawCall.mask) {
                                    if (light.isStatic) {
                                        if (!lightAabb[j]) {
                                            lightAabb[j] = new cc3d.BoundingBox();
                    ...
                    for (k = 0; k < elems.length; k++) {
                        if (elems[k].name === cc3d.SEMANTIC_POSITION) {
                            offsetP = elems[k].offset / 4; // / 4 because float
                        }
                    }
                    // #ifdef PROFILER
                    readMeshTime += cc3d.now() - subReadMeshTime;
                    // #endif

                    // #ifdef PROFILER
                    subTriAabbTime = cc3d.now();
                    // #endif
                    ...
                    // #ifdef PROFILER
                    triAabbTime += cc3d.now() - subTriAabbTime;
                    // #endif

                    // #ifdef PROFILER
                    subSearchTime = cc3d.now();
                    // #endif
                    ...
                    // #ifdef PROFILER
                    searchTime += cc3d.now() - subSearchTime;
                    // #endif

                    if (triLightCombUsed) {//.used) {

                        // #ifdef PROFILER
                        subCombineTime = cc3d.now();
                        // #endif
                        ...
                        // #ifdef PROFILER
                        combineTime += cc3d.now() - subCombineTime;
                        // #endif

                        // #ifdef PROFILER
                        subWriteMeshTime = cc3d.now();
                        // #endif

                        for (combIbName in combIndices) {
                            combIb = combIndices[combIbName];
                            var ib = new cc3d.IndexBuffer(device, indexBuffer.format, combIb.length, indexBuffer.usage);
                        ...
                        // #ifdef PROFILER
                        writeMeshTime += cc3d.now() - subWriteMeshTime;
                        // #endif
                    } else {
                        newDrawCalls.push(drawCall);
                    }
            ...
            // #ifdef PROFILER
            scene._stats.lastStaticPrepareFullTime = cc3d.now() - prepareTime;
         ...
         * @private
         * @function
         * @name cc3d.ForwardRenderer#render
         * @description Renders the scene using the specified camera.
         * @param {cc3d.Scene} scene The scene to render.
         * @param {cc3d.Camera} camera The camera with which to render the scene.
         */
        render: function (scene, camera) {
            ...
            if (target) {
                ...
                if (format === cc3d.PIXELFORMAT_RGB16F || format === cc3d.PIXELFORMAT_RGB32F) {
                    isHdr = true;
                    scene._gammaCorrection = cc3d.GAMMA_NONE;
                    scene._toneMapping = cc3d.TONEMAP_LINEAR;
                    scene.exposure = 1;
                }
            }
