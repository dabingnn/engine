...
function typeToCompnents(bufferType) {
...
function toComponentTypeCC3D ( compnentType ) {
...
function toTextureFormatCC3D ( format, type ) {
...
function parseCurves(accessor, device, bufferViews ) {
    ...
    var curverKeysView;
    var componentCount = typeToCompnents(accessor.type);
        ...
        [
            ...
            if ( accessor ) {
                vertexDesc.push({
                    ...
                    components: typeToCompnents(accessor.type),
                    type: toComponentTypeCC3D(accessor.componentType)
                });
            }
        });
...
function loadScene (url, callback) {
        ...
        loadBuffers(json, function (err, bufferViews) {
            ...
            for ( var id in json.textures ) {
                ...
                textures[id] = new cc3d.Texture(device, {
                    format: toTextureFormatCC3D(gltfTexture.format, gltfTexture.type),
                });
            }
            ...
            loadTextures(json, function (err, images) {
                for ( var id in images ) {
                    ...
                    texture.minFilter = toFilterCC3D(gltfSampler.minFilter);
                    texture.magFilter = toFilterCC3D(gltfSampler.magFilter);
                    texture.addressU = toAddressCC3D(gltfSampler.wrapS);
                    texture.addressV = toAddressCC3D(gltfSampler.wrapT);
